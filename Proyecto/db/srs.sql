...
CREATE TABLE IF NOT EXISTS `srs`.`historico` (
  ...
  INDEX `fk_historico_usuario1_idx` (`usuario_iduser` ASC),
...
CREATE TABLE IF NOT EXISTS `srs`.`archivo` (
  ...
  `alumno_has_pendiente_usuario_idalumno` INT NOT NULL,
  `alumno_has_pendiente_pendiente_idpendiente` INT NOT NULL,
  PRIMARY KEY (`idarchivo`),
  UNIQUE INDEX `ubicacion_UNIQUE` (`ubicacion` ASC),
  INDEX `fk_archivo_alumno_has_pendiente1_idx` (`alumno_has_pendiente_usuario_idalumno` ASC, `alumno_has_pendiente_pendiente_idpendiente` ASC),
  CONSTRAINT `fk_archivo_alumno_has_pendiente1`
    FOREIGN KEY (`alumno_has_pendiente_usuario_idalumno` , `alumno_has_pendiente_pendiente_idpendiente`)
    REFERENCES `srs`.`alumno_has_pendiente` (`usuario_idalumno` , `pendiente_idpendiente`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
