...
CREATE TABLE IF NOT EXISTS `srs`.`alumno_tiene_pendiente` (
  ...
  PRIMARY KEY (`idalumno`, `idpendiente`),
...
CREATE TABLE IF NOT EXISTS `srs`.`archivo` (
  ...
  INDEX `fk_archivo_alumno_has_pendiente1_idx` (`idalumno` ASC, `idpendiente` ASC),
  CONSTRAINT `fk_archivo_alumno_has_pendiente1`
    FOREIGN KEY (`idalumno` , `idpendiente`)
    REFERENCES `srs`.`alumno_tiene_pendiente` (`idalumno` , `idpendiente`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
